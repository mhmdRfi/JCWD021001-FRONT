  ...
  import { IconSearch, IconGraphFilled, IconPlus, IconArrowRight, IconArrowLeft, IconLayoutGrid, IconList, IconSortAscending2, IconSortDescending2, IconAbc, IconTags, IconChecks, IconError404, IconTrashx, IconEditCircle, IconArrowNarrowDown, IconTrashX, IconInfoCircleFilled } from '@tabler/icons-react'
  ...
  const fetchReport = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/report/sales-report?startDate=${startDate}&endDate=${endDate}`)
    ...
    const exportToPDF = async () => {
      if (salesData && reportData) {
          ...
          content: [
            ...
            '\n',
            {
              table: {
                ...
                body: [
                  ['Date', 'Total Sales', 'Total Quantity', 'Total Transactions'],
                  ...salesData.map((item) => [
                    item.saleDate,
              ...
              table: {
                ...
                body: [
                  ...
                  ...groupedReportData.map((group) => [
                    ...
                    group.products.join(', '),
                    group.transaction_items.map((transactionItem) => transactionItem.id).join(', '),
                  ]),
                ],
              },
            },
        ...
        pdfMake.createPdf(docDefinition).download('sales_data_report.pdf');
      }
    };
    ...
    const getMostSoldProducts = (groupedReportData) => {
      ...
      const mostSoldProducts = Object.keys(productsSold).map((productName) => ({
        name: productName,
    ...
    const groupByTransactionId = (reportData) => {
      ...
      reportData.forEach((item) => {
        ...
        item.transaction_item.forEach((transactionItem) => {
          const existingTransactionItem = groupedData[item.id].transaction_items.find(
            (existingItem) => existingItem.id === transactionItem.id
          );
    
          if (!existingTransactionItem) {
            groupedData[item.id].transaction_items.push(transactionItem);
  ...
  Promise.all([fetchData(), fetchReport()]);
    ...
    <Box p="20px" boxShadow='0px 1px 5px gray'>
      {salesData && salesData.length > 0 ? (<><HStack mb='5px' textAlign='center'>
    <Text fontWeight='bold' width='20%'>Sales Date</Text>
    <Spacer /> 
    <Text fontWeight='bold' width='20%'>Revenue</Text>
    <Spacer /> 
    <Text fontWeight='bold' width='20%'>Products Sold</Text>
    <Spacer /> 
    <Text fontWeight='bold' width='20%'>Transactions</Text>
  </HStack>
  <Box as='hr' borderTopWidth='5px' borderTopColor='black.200'></Box>  
  {salesData && salesData?.map((item, index) => (
                    <>
                    <HStack m='10px' textAlign='center'>
                    <Text width='20%' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' >{item?.saleDate}</Text>
                    <Spacer />
                    <Text width='20%' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' >{formatPriceToIDR(item?.totalSales)}</Text>
                    <Spacer />
                    <Text width='20%' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' >{item?.totalQuantity}</Text>
                    <Spacer />
                    <Text width='20%' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' >{item?.totalTransactions}</Text>
                  </HStack>
                  <Box as='hr' borderTopWidth='2px' borderTopColor='black.200' />
                  </>
              ...
              <Box mt='20px' p="20px" boxShadow='0px 1px 5px gray'>
                {reportData && reportData.lenth > 0 ? (<>
                  <HStack mb='5px' textAlign='center'>
    <Text fontWeight='bold' width='10%'>ID</Text>
    <Spacer /> 
    <Text fontWeight='bold' width='10%'>Quantity</Text>
    <Spacer /> 
    <Text fontWeight='bold' width='30%'>Payment</Text>
    <Spacer /> 
    <Text fontWeight='bold' width='30%'>Date</Text>
    <Text width='5%'></Text>
  </HStack>
  <Box as='hr' borderTopWidth='5px' borderTopColor='black.200'></Box>  
  ...
  {reportData && reportData?.map((item, index) => (
                    <>
                    <HStack m='10px' textAlign='center'>
                    <Text width='10%' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' >{item?.id}</Text>
                    <Spacer />
                    <Text width='10%' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' >{item?.total_quantity}</Text>
                    <Spacer />
                    <Text width='30%' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' >{formatPriceToIDR(item?.total_price)}</Text>
                    <Spacer />
                    <Text width='30%' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' >{formatISODate(item?.date)}</Text>
                    <IconButton width='5%' icon={<IconInfoCircleFilled />} onClick={() => window.open(`/transaction-detail/${item.id}`, "_blank") }/>
                  </HStack>
                  <Box as='hr' borderTopWidth='2px' borderTopColor='black.200' />
                  </>
                    
              ))}</Box> </>) : (<><Text>No Data Available</Text></>)}</Box>
    
  <Flex flexDir={{md: 'row', sm: 'column'}} mt='10px' columnGap='10px'>
          <Box width={{md: '40%', sm: '100%'}} mb='10px' p="20px" boxShadow='0px 1px 5px gray' borderRadius='10px'>
              <FormLabel>Revenue</FormLabel>
              <Heading mb='5px'>{formatPriceToIDR(totalRevenue)}</Heading>

          </Box>
          <Box width={{md: '30%', sm: '100%'}} mb='10px' p="20px" boxShadow='0px 1px 5px gray' borderRadius='10px'>
      ...
      <Box width='70vw'>
      <Line id={"chartSales"} options={options} data={{ labels, datasets: [data.datasets[0]] }} />
      <Line id={"chartProducts"} options={options} data={{ labels, datasets: [data.datasets[1]] }} />
      <Line id={"chartTransactions"} options={options} data={{ labels, datasets: [data.datasets[2]] }} />
    </Box>
          </Box>
      </Box>
