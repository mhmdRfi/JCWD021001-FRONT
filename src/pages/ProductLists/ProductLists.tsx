        ...
        content: [
          ...
          '\n',
          {
            table: {
              ...
              body: [
                ['ID', 'Name', 'SKU', 'Price', 'MarkUp (%)', 'Description', 'Status', 'Quantity', 'Created At', 'Updated At'],
...
const getPageNumbers = () => {
  ...
  return (
    ...
    <Box width='98.7vw' height='fit-content' backgroundColor='#fbfaf9' p='50px'>
        ...
        <HStack mb='30px'><InputGroup>
    <InputLeftElement pointerEvents='none'>
      <IconSearch color='black' />
    </InputLeftElement>
    <Input type='tel' placeholder='Search' width='50vw' borderRadius='full' borderColor='solid grey 1px' onChange={(e) => handleProductName(e.target.value)} />
  </InputGroup><Spacer /></HStack>
  <HStack mb='10px'>
    ...
    <Button rightIcon={<IconPlus />} borderRadius='full' backgroundColor='#286043' textColor='white' border='solid 1px #286043' onClick={() => navigate('/add-product')}>Add Item</Button>
  </HStack>
  {data?.products?.length == 0 ? (<><VStack><IconError404 color='#286043' size='200px' /><Heading color='#286043'>Product Not Found</Heading></VStack></>) : (<>  {viewMode === "list" ? (<Box p="20px" boxShadow='0px 1px 5px gray'><HStack mb='5px'>
...
{data?.products && data?.products.map((item, index) => (
                   ...
                   <Image
                      ...
                      justifySelf='center' />
                   <Spacer /> 
                   <Text width='210px' isTruncated textOverflow='ellipsis' whiteSpace='nowrap' onClick={() => navigate(`/product-detail/${item.id}` )}>{item?.name}</Text>
                   <Spacer /> 
                    <Text width='120px' textOverflow='ellipsis' whiteSpace='nowrap'>{formatPriceToIDR(item?.price)}</Text>
                   {item.quantity == 0 ? (
                          <Box backgroundColor='#fce8ed' fontSize='small' ml='130px' mr='100px' textColor='#dd1c49' p='2px' borderRadius='5px' border='solid 1px #dd1c49'>Out of Stock</Box>
                        ) : (<Text ml='140px' mr='140px'>{item.quantity}</Text>)}
                   <IconButton  icon={<IconEditCircle />} variant='ghost' colorScheme='blue' onClick={() => navigate(`/edit-product/${item.id}`)} />
                   <IconButton  icon={<IconTrashX />} variant='ghost' colorScheme='red' onClick={() => handleDeleteProduct(item)} />
                 </HStack>
            ...
            {data?.products &&
              data?.products.map((item, index) => (
                ...
                <Card key={item.id} maxW='240px' onClick={() => handleItemClick(item.id)} bg={useColorModeValue('white', 'gray.800')}
              ...
              <Image
                  ...
                  <CardFooter>
                    <ButtonGroup spacing='2'>
                      <Button variant='solid' bgColor='#286043' color='white' onClick={() => navigate(`/product-detail/${item.id}` )}>
                        Detail
                      </Button>
                      <Button variant='ghost' color='#286043' onClick={() => navigate(`/edit-product/${item.id}`)}>
                        Update
                      </Button>
                    </ButtonGroup>
                  </CardFooter>
          ...
          <HStack marginTop='10px'>
            ...
            <Select width='fit-content' placeholder="Select option" value={pageSize} onChange={(e) => setPageSize(e.target.value)}>
            ...
            <Spacer />
  <Button borderRadius='full' backgroundColor='#286043' textColor='white' border='solid 1px #286043' leftIcon={<IconArrowLeft />} isDisabled={page == 1 ? true : false} onClick={() => setPage(page - 1)} ></Button>
  ...
  <Button borderRadius='full' backgroundColor='#286043' textColor='white' border='solid 1px #286043' rightIcon={<IconArrowRight />} isDisabled={page == data?.totalPages ? true : false} onClick={() => setPage(page + 1)}></Button>
  </HStack></>)}
