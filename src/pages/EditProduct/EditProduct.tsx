...
const EditProduct = () => {
	...
	useEffect(() => {
		const calculatedPrice =
			Number(mainPrice) +
			(Number(mainPrice) * markupPercentage) / 100;
	...
	const priceFinal: number =
		Number(mainPrice) + (mainPrice * markupPercentage) / 100;
	console.log(priceFinal);

	console.log("ini q", quantity);
	...
	const editProduct = async () => {
		try {
			...
			formData.append("name", String(name) || data.name);
			formData.append("price", Number(priceFinal) || data?.price);
			formData.append("quantity", Number(quantity) || data?.quantity);
			formData.append(
				"description",
			...
			formData.append(
				"markup",
				Number(markupPercentage) || data.markup
			);
			formData.append("sku", String(sku) || data.sku);
	...
	const decrement = (category: any) => {
		const updatedCategories = selectedC.filter(
			(item: any) => item.category.id !== category.id
		);
									...
									{data?.categories?.map((item) => (
										...
										>
											<HStack>
												...
												<IconButton
													onClick={() =>
														deleteCategoryProduct(item.category.id)
													}
													bg="transparent"
													borderRadius="full"
													size="10px"
								...
								{selectedC.map((selectedCategory) => (
									...
									>
										<HStack>
											...
											<IconButton
												onClick={() =>
													decrement(selectedCategory.category)
												}
												bg="transparent"
												borderRadius="full"
												size="10px"
